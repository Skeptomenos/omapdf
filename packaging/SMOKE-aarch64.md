...
| CLI entry | `python3 -m omapdf.cli --version` | Confirms console scripts resolve |
| MCP import | `python3 -c "from omapdf.mcp_server import mcp"` | Optional extra |
...
omapdf --version
omapdf-mcp --help 2>/dev/null || true   # needs python-mcp optdepend
omapdf edit /path/to/sample.pdf         # needs gtk4 + python-gobject
```
...
| Desktop file | `share/omapdf.desktop` opens PDFs via `omapdf open` |
...
- Editor is not a hard `depends`; document `gtk4` + `python-gobject` for `omapdf edit` in README.
