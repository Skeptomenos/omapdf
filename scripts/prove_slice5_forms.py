...
def prove_cli(work: Path) -> dict:
    ...
    before = json.loads(
        subprocess.check_output(
            [sys.executable, "-m", "omepreview.cli", "read", str(src), "--json"],
            text=True,
        )
    )
    ...
    after = json.loads(
        subprocess.check_output(
            [sys.executable, "-m", "omepreview.cli", "read", str(out), "--json"],
            text=True,
        )
    )
    ...
    filled = json.loads(
        subprocess.check_output(
            [sys.executable, "-m", "omepreview.cli", "read", str(fill_out), "--json"],
            text=True,
        )
    )
    result = {
...
def prove_mcp(work: Path) -> dict:
    from omepreview import mcp_server, read
