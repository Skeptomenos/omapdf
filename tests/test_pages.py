...
from omapdf import engine, pages as pages_mod
from omapdf.mcp_server import delete_pages, extract_pages, insert_pages, list_pages, rotate_pages
...
def test_cli_pages_list(six_page_pdf):
    run = subprocess.run(
        [sys.executable, "-m", "omapdf.cli", "pages", str(six_page_pdf), "--list"],
        capture_output=True,
        text=True,
    )
...
def test_edit_command_still_available(six_page_pdf):
    ...
    from omapdf.cli import build_parser, cmd_edit
